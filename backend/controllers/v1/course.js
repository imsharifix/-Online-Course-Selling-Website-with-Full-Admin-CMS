...
exports.getCategoryCourses = async (req, res) => {
  ...
  // find courses in this category and populate creator and category
  const categoryCourses = await courseModel
    .find({ categoryID: category[0]._id })
    .populate("creator", "-password")
    .populate("categoryID")
    .lean();
  ...
  const mapped = categoryCourses.map((course) => {
    let courseTotalScore = 5;

    const courseRegisters = registers.filter(
      (r) => String(r.course) === String(course._id)
    );

    const courseScores = comments.filter(
      (c) => String(c.course) === String(course._id)
    );

    courseScores.forEach((c) => {
      courseTotalScore += Number(c.score || 0);
    });

    return {
      ...course,
      categoryID: course.categoryID.title,
      creator: course.creator ? course.creator.name : null,
      registers: courseRegisters.length,
